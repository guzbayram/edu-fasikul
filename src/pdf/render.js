...
async function endZoomGesture(){
  ...
  console.log('[ZOOMDEBUG] start ' + JSON.stringify({pageAnchor, myGen, liveZoom, renderedZoom}));
  ...
  console.log('[ZOOMDEBUG] after renderPages ' + JSON.stringify({myGen, _zoomSettleGen, aborted: myGen !== _zoomSettleGen}));
  ...
  console.log('[ZOOMDEBUG] freshPageEl ' + JSON.stringify({pageAnchor, hasFreshPageEl: !!freshPageEl, w: freshPageEl?.offsetWidth, h: freshPageEl?.offsetHeight, rendered: freshPageEl?.dataset?.rendered}));
  ...
  if(appState.viewMode === 'scroll' && freshPageEl && freshPageEl.dataset.rendered !== '1'){
    ...
    console.log('[ZOOMDEBUG] after force-render ' + JSON.stringify({myGen, _zoomSettleGen, aborted: myGen !== _zoomSettleGen, w: freshPageEl?.offsetWidth, h: freshPageEl?.offsetHeight}));
  ...
  if(freshPageEl && freshPageEl.offsetWidth && freshPageEl.offsetHeight){
    ...
    console.log('[ZOOMDEBUG] branch=pageEl ' + JSON.stringify({prTop: pr.top, prHeight: pr.height, fracY: pageAnchor.fracY, focalY, resultScrollTop: wrap.scrollTop}));
  } else {
    ...
    console.log('[ZOOMDEBUG] branch=ratioFallback ' + JSON.stringify({freshRectTop: freshRect.top, targetContentY, focalY, resultScrollTop: wrap.scrollTop}));
  }
  wrap.style.overflow = '';
  wrap.classList.remove('zoom-settling');
